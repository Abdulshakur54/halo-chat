...
function LeftSideBar() {
    const { getUsers, users, selectedUser, setSelectedUser, unseenMessages, setUnseenMessages, markMessagesAsSeen } = useContext(ChatContext)
        ...
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ""}`}>
            <div className='pb-5'>
                <div className='flex justify-between items-center'>
                    ...
                    <div className='relative py-2 group'>
                        <FontAwesomeIcon icon={faEllipsisVertical} className='max-h-5 cursor-pointer' />
            ...
            <div className='flex flex-col'>
                {filteredUsers.map((user, index) => (
                    <div onClick={() => markMessagesAsSeen(user)} key={index} className={`relative flex items-center gap-2 p-2 pl-4
                     rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                       
                        {user?.profilePicId ? <Image src={user.profilePicId} alt={user.name} className='w-[35px] aspect-[1/1] rounded-full' />:  
                        <img src={assets.avatarIcon} alt={user.name} className='w-[35px] aspect-[1/1] rounded-full' />}
